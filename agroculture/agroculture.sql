...
select * from fproduct where (fproduct.fid = cust_i);
END$$
...
--
DELIMITER $$
CREATE TRIGGER `trig_dist` BEFORE INSERT ON `fproduct` FOR EACH ROW BEGIN
DECLARE msg varchar(128);
if(new.price<0) THEN
set msg = 'Error: price cant be negative....' ;
signal sqlstate '45001' set message_text = msg; 
end if;
if new.pid<0 THEN
	set msg = 'Error: pid cant be negative....';
        signal sqlstate '45001' set message_text = msg;
end if;
END
$$
...
--
DELIMITER $$
CREATE TRIGGER `trig_dest` BEFORE INSERT ON `transaction` FOR EACH ROW BEGIN
